...
class PuzzleTests(unittest.TestCase):
    ...
    def test_rebus(self):
        ...
        self.assertTrue(
            all("STAR" == r.get_rebus_solution(i) for i in r.get_rebus_squares())
        )
    ...
    def test_markup(self):
        ...
        m = p.markup()
        self.assertTrue(
            all(puz.GridMarkup.Circled == m.markup[i] for i in m.get_markup_squares())
        )
        # trigger save
        p.tobytes()
    ...
    def test_puzzle_type(self):
        self.assertNotEqual(
            puz.read("testfiles/washpost.puz").puzzletype, puz.PuzzleType.Diagramless
        )
        self.assertNotEqual(
            puz.read("testfiles/nyt_locked.puz").puzzletype, puz.PuzzleType.Diagramless
        )
        self.assertEqual(
    ...
    def test_downs_only_puzzle(self):
        ...
        self.assertEqual(len(list(filter(_empty_clue, p.clue_numbering().across))), 0)
        self.assertEqual(len(list(filter(_empty_clue, p.clue_numbering().down))), 0)

        p.convert_to_downs_only()
        self.assertEqual(
            len(list(filter(_empty_clue, p.clue_numbering().across))),
